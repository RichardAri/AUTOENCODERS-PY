...
datos = pd.read_csv('/content/drive/My Drive/SI_L10_AUTOENCODERS_DATASET.csv')
...
# Distribucion de las caracteristicas V1 a V28 en normales y fraudulentos 
...
for i,cn in enumerate(v_1_28):
  sns.distplot(datos[cn][datos.Class == 1], bins=50, label='Fraudulentos', color='#F2545B')
  sns.distplot(datos[cn][datos.Class == 0], bins=50, label='Normales', color='#19323C')
  plt.xlabel("")
...
#3.1 La variable "Tiempo" no aporta informacion. La eliminaremos
...
# Prediccion X_test -> Autoencoder -> X_pred
X_pred = autoencoder.predict(X_test)
ecm = np.mean(np.power(X_pred - X_pred, 2), axis=1)
print(X_pred.shape)

# Grafica precision-recall para determinar el umbral
